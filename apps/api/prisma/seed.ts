import 'dotenv/config';
import { PrismaPg } from '@prisma/adapter-pg';
import { PrismaClient } from '@prisma/client';

const connectionString = process.env.DATABASE_URL;
if (!connectionString) {
  throw new Error('DATABASE_URL is not set');
}

const adapter = new PrismaPg({ connectionString });
const prisma = new PrismaClient({ adapter });

interface IngredientSeed {
  name: string;
  category: string;
  defaultUnit: string;
  pricePerUnit: number;
  caloriesPer100g: number;
  proteinPer100g: number;
  carbsPer100g: number;
  fatPer100g: number;
  storageInstructions: string;
}

const ingredients: IngredientSeed[] = [
  {
    name: 'Chicken breast',
    category: 'Protein',
    defaultUnit: 'g',
    pricePerUnit: 0.012,
    caloriesPer100g: 165,
    proteinPer100g: 31,
    carbsPer100g: 0,
    fatPer100g: 3.6,
    storageInstructions: 'Keep refrigerated, use within 2 days or freeze.',
  },
  {
    name: 'Rice',
    category: 'Grain',
    defaultUnit: 'g',
    pricePerUnit: 0.003,
    caloriesPer100g: 130,
    proteinPer100g: 2.7,
    carbsPer100g: 28,
    fatPer100g: 0.3,
    storageInstructions: 'Store in a cool, dry place.',
  },
  {
    name: 'Pasta',
    category: 'Grain',
    defaultUnit: 'g',
    pricePerUnit: 0.0025,
    caloriesPer100g: 131,
    proteinPer100g: 5,
    carbsPer100g: 25,
    fatPer100g: 1.1,
    storageInstructions: 'Store in a cool, dry place.',
  },
  {
    name: 'Tomato sauce',
    category: 'Condiment',
    defaultUnit: 'g',
    pricePerUnit: 0.004,
    caloriesPer100g: 29,
    proteinPer100g: 1.6,
    carbsPer100g: 6,
    fatPer100g: 0.2,
    storageInstructions: 'Refrigerate after opening.',
  },
  {
    name: 'Onion',
    category: 'Vegetable',
    defaultUnit: 'unit',
    pricePerUnit: 0.5,
    caloriesPer100g: 40,
    proteinPer100g: 1.1,
    carbsPer100g: 9,
    fatPer100g: 0.1,
    storageInstructions: 'Store in a cool, dry place.',
  },
  {
    name: 'Garlic',
    category: 'Vegetable',
    defaultUnit: 'clove',
    pricePerUnit: 0.1,
    caloriesPer100g: 149,
    proteinPer100g: 6.4,
    carbsPer100g: 33,
    fatPer100g: 0.5,
    storageInstructions: 'Store in a cool, dry place.',
  },
  {
    name: 'Olive oil',
    category: 'Condiment',
    defaultUnit: 'ml',
    pricePerUnit: 0.02,
    caloriesPer100g: 884,
    proteinPer100g: 0,
    carbsPer100g: 0,
    fatPer100g: 100,
    storageInstructions: 'Store at room temperature, away from light.',
  },
  {
    name: 'Eggs',
    category: 'Protein',
    defaultUnit: 'unit',
    pricePerUnit: 0.3,
    caloriesPer100g: 155,
    proteinPer100g: 13,
    carbsPer100g: 1.1,
    fatPer100g: 11,
    storageInstructions: 'Keep refrigerated.',
  },
  {
    name: 'Tuna',
    category: 'Protein',
    defaultUnit: 'g',
    pricePerUnit: 0.015,
    caloriesPer100g: 116,
    proteinPer100g: 26,
    carbsPer100g: 0,
    fatPer100g: 1,
    storageInstructions: 'Refrigerate after opening.',
  },
  {
    name: 'Potatoes',
    category: 'Vegetable',
    defaultUnit: 'g',
    pricePerUnit: 0.002,
    caloriesPer100g: 77,
    proteinPer100g: 2,
    carbsPer100g: 17,
    fatPer100g: 0.1,
    storageInstructions: 'Store in a cool, dark, dry place.',
  },
  {
    name: 'Greek yogurt',
    category: 'Dairy',
    defaultUnit: 'g',
    pricePerUnit: 0.008,
    caloriesPer100g: 59,
    proteinPer100g: 10,
    carbsPer100g: 3.6,
    fatPer100g: 0.4,
    storageInstructions: 'Keep refrigerated.',
  },
  {
    name: 'Lettuce',
    category: 'Vegetable',
    defaultUnit: 'g',
    pricePerUnit: 0.006,
    caloriesPer100g: 15,
    proteinPer100g: 1.4,
    carbsPer100g: 2.9,
    fatPer100g: 0.2,
    storageInstructions: 'Keep refrigerated.',
  },
  {
    name: 'Carrot',
    category: 'Vegetable',
    defaultUnit: 'unit',
    pricePerUnit: 0.3,
    caloriesPer100g: 41,
    proteinPer100g: 0.9,
    carbsPer100g: 10,
    fatPer100g: 0.2,
    storageInstructions: 'Keep refrigerated.',
  },
  {
    name: 'Milk',
    category: 'Dairy',
    defaultUnit: 'ml',
    pricePerUnit: 0.0015,
    caloriesPer100g: 61,
    proteinPer100g: 3.2,
    carbsPer100g: 4.8,
    fatPer100g: 3.3,
    storageInstructions: 'Keep refrigerated.',
  },
  {
    name: 'Cheese',
    category: 'Dairy',
    defaultUnit: 'g',
    pricePerUnit: 0.015,
    caloriesPer100g: 402,
    proteinPer100g: 25,
    carbsPer100g: 1.3,
    fatPer100g: 33,
    storageInstructions: 'Keep refrigerated.',
  },
  {
    name: 'Couscous',
    category: 'Grain',
    defaultUnit: 'g',
    pricePerUnit: 0.004,
    caloriesPer100g: 112,
    proteinPer100g: 3.8,
    carbsPer100g: 23,
    fatPer100g: 0.2,
    storageInstructions: 'Store in a cool, dry place.',
  },
  {
    name: 'Lamb',
    category: 'Protein',
    defaultUnit: 'g',
    pricePerUnit: 0.02,
    caloriesPer100g: 294,
    proteinPer100g: 25,
    carbsPer100g: 0,
    fatPer100g: 21,
    storageInstructions: 'Keep refrigerated, use within 2 days or freeze.',
  },
  {
    name: 'Chickpeas',
    category: 'Legume',
    defaultUnit: 'g',
    pricePerUnit: 0.006,
    caloriesPer100g: 164,
    proteinPer100g: 8.9,
    carbsPer100g: 27,
    fatPer100g: 2.6,
    storageInstructions: 'Store dried in a cool, dry place; soak before cooking.',
  },
  {
    name: 'Harissa',
    category: 'Condiment',
    defaultUnit: 'g',
    pricePerUnit: 0.03,
    caloriesPer100g: 65,
    proteinPer100g: 2,
    carbsPer100g: 10,
    fatPer100g: 3,
    storageInstructions: 'Refrigerate after opening.',
  },
  {
    name: 'Brick pastry sheet',
    category: 'Grain',
    defaultUnit: 'unit',
    pricePerUnit: 0.4,
    caloriesPer100g: 300,
    proteinPer100g: 8,
    carbsPer100g: 55,
    fatPer100g: 5,
    storageInstructions: 'Keep refrigerated, use within a few days of opening.',
  },
  {
    name: 'Bell pepper',
    category: 'Vegetable',
    defaultUnit: 'unit',
    pricePerUnit: 0.6,
    caloriesPer100g: 31,
    proteinPer100g: 1,
    carbsPer100g: 6,
    fatPer100g: 0.3,
    storageInstructions: 'Keep refrigerated.',
  },
  {
    name: 'Bread',
    category: 'Grain',
    defaultUnit: 'g',
    pricePerUnit: 0.003,
    caloriesPer100g: 265,
    proteinPer100g: 9,
    carbsPer100g: 49,
    fatPer100g: 3.2,
    storageInstructions: 'Store at room temperature in a bread bag.',
  },
  {
    name: 'Cumin',
    category: 'Spice',
    defaultUnit: 'g',
    pricePerUnit: 0.03,
    caloriesPer100g: 375,
    proteinPer100g: 18,
    carbsPer100g: 44,
    fatPer100g: 22,
    storageInstructions: 'Store in a cool, dry place, away from light.',
  },
  {
    name: 'Parsley',
    category: 'Vegetable',
    defaultUnit: 'g',
    pricePerUnit: 0.02,
    caloriesPer100g: 36,
    proteinPer100g: 3,
    carbsPer100g: 6,
    fatPer100g: 0.8,
    storageInstructions: 'Keep refrigerated, wrapped in a damp towel.',
  },
  {
    name: 'Black olives',
    category: 'Condiment',
    defaultUnit: 'g',
    pricePerUnit: 0.012,
    caloriesPer100g: 115,
    proteinPer100g: 0.8,
    carbsPer100g: 6,
    fatPer100g: 11,
    storageInstructions: 'Keep refrigerated after opening, submerged in brine.',
  },
  {
    name: 'Lemon juice',
    category: 'Condiment',
    defaultUnit: 'ml',
    pricePerUnit: 0.01,
    caloriesPer100g: 22,
    proteinPer100g: 0.4,
    carbsPer100g: 6.9,
    fatPer100g: 0.2,
    storageInstructions: 'Keep refrigerated after opening.',
  },
];

interface MealSeed {
  name: string;
  description: string;
  cookingTimeMinutes: number;
  difficulty: string;
  cuisine: string;
  dietTags: string[];
  instructions: string;
  isPublished: boolean;
  ingredients: { name: string; quantity: number; unit: string }[];
}

const meals: MealSeed[] = [
  {
    name: 'Chicken Rice Bowl',
    description:
      'A high-protein bowl with seasoned chicken breast, rice, and a yogurt sauce.',
    cookingTimeMinutes: 30,
    difficulty: 'easy',
    cuisine: 'Fusion',
    dietTags: ['high-protein'],
    instructions:
      '1. Cook rice. 2. Season and pan-sear chicken breast. 3. Saute onion and garlic in olive oil. 4. Slice chicken and serve over rice with yogurt sauce.',
    isPublished: true,
    ingredients: [
      { name: 'Chicken breast', quantity: 250, unit: 'g' },
      { name: 'Rice', quantity: 150, unit: 'g' },
      { name: 'Onion', quantity: 1, unit: 'unit' },
      { name: 'Garlic', quantity: 2, unit: 'clove' },
      { name: 'Olive oil', quantity: 10, unit: 'ml' },
      { name: 'Greek yogurt', quantity: 80, unit: 'g' },
    ],
  },
  {
    name: 'Tuna Pasta',
    description:
      'Pasta tossed with tuna, tomato sauce, and sauteed onion and garlic.',
    cookingTimeMinutes: 25,
    difficulty: 'easy',
    cuisine: 'Italian',
    dietTags: ['high-protein'],
    instructions:
      '1. Boil pasta. 2. Saute onion and garlic in olive oil. 3. Add tomato sauce and tuna, simmer. 4. Toss with cooked pasta.',
    isPublished: true,
    ingredients: [
      { name: 'Pasta', quantity: 200, unit: 'g' },
      { name: 'Tuna', quantity: 150, unit: 'g' },
      { name: 'Tomato sauce', quantity: 100, unit: 'g' },
      { name: 'Onion', quantity: 1, unit: 'unit' },
      { name: 'Garlic', quantity: 2, unit: 'clove' },
      { name: 'Olive oil', quantity: 10, unit: 'ml' },
    ],
  },
  {
    name: 'Vegetable Omelette',
    description: 'Fluffy eggs folded with carrot, onion, and melted cheese.',
    cookingTimeMinutes: 15,
    difficulty: 'easy',
    cuisine: 'French',
    dietTags: ['vegetarian'],
    instructions:
      '1. Whisk eggs with milk. 2. Saute onion and carrot in olive oil. 3. Pour eggs into the pan, add cheese, and fold once set.',
    isPublished: true,
    ingredients: [
      { name: 'Eggs', quantity: 3, unit: 'unit' },
      { name: 'Milk', quantity: 30, unit: 'ml' },
      { name: 'Onion', quantity: 0.5, unit: 'unit' },
      { name: 'Carrot', quantity: 1, unit: 'unit' },
      { name: 'Cheese', quantity: 30, unit: 'g' },
      { name: 'Olive oil', quantity: 5, unit: 'ml' },
    ],
  },
  {
    name: 'Greek Yogurt Chicken Wrap',
    description: 'Seasoned chicken with a cool yogurt sauce and crisp lettuce.',
    cookingTimeMinutes: 20,
    difficulty: 'easy',
    cuisine: 'Mediterranean',
    dietTags: ['high-protein'],
    instructions:
      '1. Season and pan-sear chicken breast. 2. Mix yogurt with garlic and olive oil for the sauce. 3. Slice chicken and assemble with lettuce and sauce.',
    isPublished: true,
    ingredients: [
      { name: 'Chicken breast', quantity: 200, unit: 'g' },
      { name: 'Greek yogurt', quantity: 100, unit: 'g' },
      { name: 'Lettuce', quantity: 50, unit: 'g' },
      { name: 'Onion', quantity: 0.5, unit: 'unit' },
      { name: 'Garlic', quantity: 1, unit: 'clove' },
      { name: 'Olive oil', quantity: 5, unit: 'ml' },
    ],
  },
  {
    name: 'Potato Egg Skillet',
    description:
      'A hearty skillet of pan-fried potatoes, eggs, onion, and cheese.',
    cookingTimeMinutes: 30,
    difficulty: 'medium',
    cuisine: 'Comfort food',
    dietTags: ['vegetarian'],
    instructions:
      '1. Dice and pan-fry potatoes in olive oil until golden. 2. Add onion and cook until soft. 3. Crack in eggs and cook to preference. 4. Top with cheese.',
    isPublished: true,
    ingredients: [
      { name: 'Potatoes', quantity: 300, unit: 'g' },
      { name: 'Eggs', quantity: 2, unit: 'unit' },
      { name: 'Onion', quantity: 0.5, unit: 'unit' },
      { name: 'Cheese', quantity: 30, unit: 'g' },
      { name: 'Olive oil', quantity: 10, unit: 'ml' },
    ],
  },
  {
    name: 'Healthy Chicken Salad',
    description:
      'A light salad of grilled chicken, lettuce, and carrot with a yogurt dressing.',
    cookingTimeMinutes: 20,
    difficulty: 'easy',
    cuisine: 'Mediterranean',
    dietTags: ['high-protein', 'low-carb'],
    instructions:
      '1. Season and grill chicken breast. 2. Toss lettuce and carrot. 3. Slice chicken over the salad. 4. Drizzle with yogurt and olive oil dressing.',
    isPublished: true,
    ingredients: [
      { name: 'Chicken breast', quantity: 200, unit: 'g' },
      { name: 'Lettuce', quantity: 100, unit: 'g' },
      { name: 'Carrot', quantity: 1, unit: 'unit' },
      { name: 'Cheese', quantity: 30, unit: 'g' },
      { name: 'Olive oil', quantity: 10, unit: 'ml' },
      { name: 'Greek yogurt', quantity: 50, unit: 'g' },
    ],
  },
  {
    name: 'Tunisian Couscous',
    description:
      'Traditional semolina couscous steamed with tender lamb, carrots, and chickpeas in a spiced harissa tomato broth.',
    cookingTimeMinutes: 90,
    difficulty: 'hard',
    cuisine: 'Tunisian',
    dietTags: ['high-protein'],
    instructions:
      '1. Brown lamb pieces in olive oil with onion and garlic. 2. Add tomato sauce, harissa, and water, then simmer until tender. 3. Add carrots and chickpeas, cook until soft. 4. Steam couscous and fluff with a little olive oil. 5. Serve couscous topped with the lamb, vegetables, and broth.',
    isPublished: true,
    ingredients: [
      { name: 'Lamb', quantity: 300, unit: 'g' },
      { name: 'Couscous', quantity: 200, unit: 'g' },
      { name: 'Carrot', quantity: 2, unit: 'unit' },
      { name: 'Chickpeas', quantity: 100, unit: 'g' },
      { name: 'Tomato sauce', quantity: 150, unit: 'g' },
      { name: 'Harissa', quantity: 15, unit: 'g' },
      { name: 'Onion', quantity: 1, unit: 'unit' },
      { name: 'Garlic', quantity: 2, unit: 'clove' },
      { name: 'Olive oil', quantity: 15, unit: 'ml' },
    ],
  },
  {
    name: 'Brik a l\'oeuf',
    description:
      'Crispy pastry parcel filled with egg, tuna, and parsley, pan-fried until golden.',
    cookingTimeMinutes: 15,
    difficulty: 'easy',
    cuisine: 'Tunisian',
    dietTags: ['high-protein'],
    instructions:
      '1. Lay out a brick pastry sheet and place tuna and parsley in the center. 2. Crack an egg on top. 3. Fold the pastry into a triangle, sealing the egg inside. 4. Pan-fry in olive oil until crisp and golden on both sides.',
    isPublished: true,
    ingredients: [
      { name: 'Brick pastry sheet', quantity: 1, unit: 'unit' },
      { name: 'Eggs', quantity: 1, unit: 'unit' },
      { name: 'Tuna', quantity: 60, unit: 'g' },
      { name: 'Parsley', quantity: 5, unit: 'g' },
      { name: 'Olive oil', quantity: 10, unit: 'ml' },
    ],
  },
  {
    name: 'Lablabi',
    description:
      'A warming chickpea soup over torn bread, spiced with cumin, garlic, and harissa, topped with a poached egg.',
    cookingTimeMinutes: 25,
    difficulty: 'easy',
    cuisine: 'Tunisian',
    dietTags: ['vegetarian'],
    instructions:
      '1. Simmer chickpeas with garlic, cumin, and harissa in their broth. 2. Tear bread into a bowl. 3. Ladle the hot chickpea soup over the bread. 4. Top with a poached egg and a drizzle of olive oil.',
    isPublished: true,
    ingredients: [
      { name: 'Chickpeas', quantity: 200, unit: 'g' },
      { name: 'Bread', quantity: 80, unit: 'g' },
      { name: 'Garlic', quantity: 2, unit: 'clove' },
      { name: 'Cumin', quantity: 3, unit: 'g' },
      { name: 'Harissa', quantity: 10, unit: 'g' },
      { name: 'Eggs', quantity: 1, unit: 'unit' },
      { name: 'Olive oil', quantity: 10, unit: 'ml' },
    ],
  },
  {
    name: 'Salade Mechouia',
    description:
      'Smoky grilled peppers and tomato sauce blended with garlic, topped with tuna, egg, and olives.',
    cookingTimeMinutes: 20,
    difficulty: 'easy',
    cuisine: 'Tunisian',
    dietTags: ['high-protein', 'low-carb'],
    instructions:
      '1. Grill bell peppers until charred, then peel and chop. 2. Mix with tomato sauce, garlic, and olive oil. 3. Season with cumin. 4. Top with tuna, hard-boiled egg, and black olives, and a squeeze of lemon juice.',
    isPublished: true,
    ingredients: [
      { name: 'Bell pepper', quantity: 2, unit: 'unit' },
      { name: 'Tomato sauce', quantity: 50, unit: 'g' },
      { name: 'Garlic', quantity: 1, unit: 'clove' },
      { name: 'Cumin', quantity: 2, unit: 'g' },
      { name: 'Olive oil', quantity: 15, unit: 'ml' },
      { name: 'Tuna', quantity: 80, unit: 'g' },
      { name: 'Eggs', quantity: 1, unit: 'unit' },
      { name: 'Black olives', quantity: 20, unit: 'g' },
      { name: 'Lemon juice', quantity: 10, unit: 'ml' },
    ],
  },
];

async function main() {
  console.log('Seeding ingredients...');
  const ingredientRecords = new Map<string, string>();

  for (const ingredient of ingredients) {
    const record = await prisma.ingredient.upsert({
      where: { name: ingredient.name },
      update: ingredient,
      create: ingredient,
    });
    ingredientRecords.set(record.name, record.id);
  }

  console.log('Seeding meals...');
  for (const meal of meals) {
    const { ingredients: mealIngredients, ...mealData } = meal;

    const existing = await prisma.meal.findFirst({
      where: { name: meal.name },
    });

    const mealRecord = existing
      ? await prisma.meal.update({ where: { id: existing.id }, data: mealData })
      : await prisma.meal.create({ data: mealData });

    for (const mealIngredient of mealIngredients) {
      const ingredientId = ingredientRecords.get(mealIngredient.name);
      if (!ingredientId) {
        throw new Error(`Unknown ingredient: ${mealIngredient.name}`);
      }

      await prisma.mealIngredient.upsert({
        where: {
          mealId_ingredientId: {
            mealId: mealRecord.id,
            ingredientId,
          },
        },
        update: {
          quantity: mealIngredient.quantity,
          unit: mealIngredient.unit,
        },
        create: {
          mealId: mealRecord.id,
          ingredientId,
          quantity: mealIngredient.quantity,
          unit: mealIngredient.unit,
        },
      });
    }
  }

  console.log(
    `Seeded ${ingredients.length} ingredients and ${meals.length} meals.`,
  );
}

main()
  .catch((error) => {
    console.error(error);
    process.exitCode = 1;
  })
  .finally(async () => {
    await prisma.$disconnect();
  });
